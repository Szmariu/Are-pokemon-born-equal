...
Since I played the Pokemon Red 10 years ago on my Gameboy Advance, I wondered about the statistics that define different pokemon. Are the fair? Are some pokemon just better than others, or maybe there are some groups, or specializations? When I encountered the pokemon dataset on Kaggle, I knew this was a chance to answer this questions.   
The goal of this paper is to test what distributions the pokemon statlines follow. I want to also see if there are any significant groups within the population, or are the statistics assignes semi-randomnly.  
...
To display the sihouette plot, we can do the clustering using eclust().
...
Aditionally, we can run the PAM algorithm on the MDS and PCA points. Since this is a small dataset, the results are very simillar to K-means. 
...
After this analisys I can conclude, that there are rules governing the assigment of statisics to different pokemon.  

Visual analisys of the population showed that there is a strong two-peak distribution in the Total statistic, corresponding to pre and post evolution pokemon. It was shown, that the highest values are acheived by the small subset of all pokemon, the legendary pokemon. The analisys shown no signs of *"power creep"* between generations and showed that all statistics other than Total follow a normal distribution aroun 55.   

Multidimensional scaling analisys was able to correctly identify significant outliers, that were then confirmed with a outside source.  

MDS analisys, later confirmed by PCA, showed that the more powerful pokemon fall into 3 different categories - **Agresive** (high Speed and Special Attack), **Defenisive** (high Defense and Special Defense) and **Well-rounded** (high HP and Attack). 

The K-means and PAM analisys showed that the dataset can be divided into 5 groups - **‘weak’**, **‘medium - average’**, **‘medium - aggresive’**, **‘medium - defensive’** and **‘legendary’** pokemon. 
